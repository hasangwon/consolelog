  ...
  useEffect(() => {
    firestore
      ...
      .then((doc: any) => {
        setBoards([]);
        doc.forEach((val: any) => {
          setBoards((board) => [...board, val.data()]);
        });
      });
  }, []);
        ...
        <div className="flex flex-col bg-gray-600 w-[100%] ">
          <div className="flex justify-between items-center border h-[2rem] bg-gray-700">
            ...
            <div>
              <input
                type="search"
                className="h-[1.5rem] bg-gray-800 border brder-gray-100 px-3"
          ...
          <div className="flex flex-row border flex-1">
            <div className="flex border">
              <ul className="w-[3rem] bg-gray-800">
                {boards.map((val) => (
                  <li key={val.key} className="py-4 text-gray-400">
                    {val.name}
                  </li>
                ))}
              </ul>
